...
constexpr std::size_t max_path_len = 108;
...
inline unix_addr buildAddr(std::span<const std::byte, max_path_len> path)
{
    sockaddr_un addr{};
    ...
    if (path.front() == std::byte{0}) { // ABSTRACT
        auto safe_view = path | std::views::take(sizeof(addr.sun_path));
        ...
        auto safe_view = path | std::views::take(sizeof(addr.sun_path) - 1);
        auto [in, out] = std::ranges::copy(
            safe_view | std::views::transform([](std::byte b) { return static_cast<char>(b); }),
            span_path.data());
